...
public class AwaitCyclicBarrier implements IStatement{

    /*
        AwaitCyclicBarrier class implements abstract methods of IStatement interface
     */

    private final String var;

    public AwaitCyclicBarrier(String var){
        /*
            Parametrised constructor which creates an AwaitCyclicBarrier object
    ...
    public ProgramState execute (ProgramState state) throws Exception{
        /*
            Simulates await()
            Retrieves the entry from BarrierTable by using 'var' variable
            and checks how many threads are waiting at CyclicBarrier in order
            to know whether to release them and continue running or not
        ...
        if(symbolTable.isDefined(var)){
            ...
            if (val.getType().equals(new IntType())) {
                ...
                synchronized (cyclicBarrier){
                    if(cyclicBarrier.contains(foundIndex)){
                        Pair<Integer, ArrayList<Integer>> pair = cyclicBarrier.lookUp(foundIndex);
    ...
    public IStatement deepCopy(){
        return new AwaitCyclicBarrier(var);
    };
